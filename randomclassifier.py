...
import anal_util as au   ### file not included in this repo
from matplotlib import pyplot as plt
import operator
from matplotlib.colors import ListedColormap

## Graph the output using contour graph
def plot_predictions(max_feat, train_test, predict_dict, neurtypes, feature_order,epoch):
...
def run_cluster_analysis(param_values, labels, num_features, alldf, epoch, max_plots):
...
## param small: if small=True, use num_fits from each optimization, else, use %tile
def set_up_df(neurtypes, path_root, tile=0.005, num_fits=None, small=True): 
    # set of data files from parameter optimization
    pattern = path_root+'*.npz'

    # retrieve data files and sort the files by which neurtype
...
param_values, labels, num_features, alldf = set_up_df(neurtypes, path_root, tile, num_fits, True)
...
for epoch in range(0, epochs):
    features, max_feat = run_cluster_analysis(param_values, labels, num_features, alldf, epoch, max_plots)
